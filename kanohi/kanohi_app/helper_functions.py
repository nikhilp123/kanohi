...
def search_user_function(params):
    try:
        ...
        return JsonResponse({"validation": "search not found", "status": False})
#---------------------------------------------------------------------------------------------------------- 
def search_franchisee_function(params):
    try:
        search_string=params.get("search_string")
        is_active=params.get("is_active")

        args = Q()
        args = args | Q(first_name__icontains=search_string) | Q(id=search_string)| Q(franchisee=search_string)

        kwargs = {}
        kwargs["role"]=UserDetails.FRANCHISEE_ADMIN
        kwargs['is_active'] = is_active
